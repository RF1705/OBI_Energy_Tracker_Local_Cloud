...
#include "ecotracker.h"           // EcoTracker IR emulation (EcoFlow STREAM grid-meter input)
...
<div class=wrap>
 ...
 <div class=card>
  <h3>🔋 <span id=heco>EcoFlow STREAM (EcoTracker-Emulation)</span></h3>
  <div class=stat id=ecostat>…</div>
  <div class=msg id=econote style="margin:2px 0 8px"></div>
  <label style="display:flex;align-items:center;gap:8px;margin-top:12px;cursor:pointer">
   <input type=checkbox id=ecoon style="width:auto;margin:0"><span id=lecoon>Aktiviert</span></label>
  <div class=grid>
...
$('heco').textContent=t('EcoFlow STREAM (EcoTracker-Emulation)','EcoFlow STREAM (EcoTracker emulation)');
$('lecoon').textContent=t('Aktiviert — als everHome EcoTracker im Netzwerk ankündigen','Enabled — advertise as an everHome EcoTracker on the network');
$('econote').textContent=t('Macht einen Reader für einen EcoFlow STREAM (Ultra / Ultra X / AC Pro) als lokales Netzanschluss-Messgerät sichtbar: in der EcoFlow-App als externes Strommessgerät vom Typ „ECOTRACKER IR" hinzufügen — komplett lokal, ohne Cloud und ohne Zusatzgerät. GET /v1/json ist dann ohne Login erreichbar (nur die Messwerte dieses Readers).','Exposes one reader to an EcoFlow STREAM (Ultra / Ultra X / AC Pro) as its local grid meter: add it in the EcoFlow app as an external power meter of type "ECOTRACKER IR" — fully local, no cloud, no extra hardware. GET /v1/json is then reachable without login (readings of that one reader only).');
$('lecordr').textContent=t('Reader (Netzanschluss-Zähler)','Reader (grid meter)');
$('oecoauto').textContent=t('Automatisch (erster gebundener Reader)','Automatic (first bound reader)');
$('lecosrc').textContent=t('Leistungswert','Power value');
...
async function loadEcoReaders(){try{const rs=await(await fetch('/api/readers')).json();const o=$('ecordr');
 rs.filter(r=>r.assigned).forEach(r=>{const op=document.createElement('option');op.value=r.id;op.textContent=r.id+(r.name?' — '+r.name:'');o.appendChild(op);});
...
load().then(loadEcoReaders);ghCheck();setInterval(()=>{if(!ghBusy)load();},5000);   // keep WiFi/MQTT status + the device clock fresh -- but not while a GitHub OTA is running, see ghUpdate()
</script></body></html>)HTML";
...
static void startServices() {
  ...
  // EcoTracker emulation: /v1/json is deliberately UNguarded — the EcoFlow inverter polling it is a
  // headless ESP32 that cannot log in. It only ever exposes one reader's W/Wh readings, is off by
  // default, and answers 404 while disabled. The config endpoint stays behind the guard like the rest.
  server.on("/v1/json", HTTP_GET, eco_handle_v1json);
  server.on("/api/ecotracker", HTTP_POST, guard(eco_handle_cfg));
...
static void webTask(void *) {
  ...
  eco_init(&server);                 // load the EcoTracker-emulation config before its routes register
  ...
  for (;;) {
    ...
    if (g_serverUp) {
      ...
      { static uint32_t lastEcoMs = 0; uint32_t nowMs = millis();   // keep the EcoTracker mDNS record in step
        if (nowMs - lastEcoMs > 1000) { lastEcoMs = nowMs; eco_service(conn); } }   // with WiFi/IP changes
    }
